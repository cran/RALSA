...
-   The table averages in the `lsa.corr` lack the *t*- and *p*-values.

-   `lsa.download.data` reports download of main study files when bridge study files are downloaded in studies like PIRLS, TIMSS and ICCS.

## New functionality

-   The effective degrees of freedom in `lsa.lin.reg` and `lsa.corr` are now computed using the Welch-Satterthwaite approximation or Johnson-Rust correction (default, recommended).

## Miscellaneous

-   Updated TALIS 2024 workflow.
