  ...
  tryCatch({
    ...
    if(PISApre15 == FALSE) {
      if(missing(x = ISO)) {
        ...
        if(any(sapply(X = ISO, FUN = nchar) == 12 ) | any(sapply(X = ISO, FUN = nchar) == 13 )) {
          full.inp.file.path <- file.path(inp.folder, ISO)
          study.and.cycle <- unique(x = tolower(substr(x = ISO, start = 7, stop = 9)))
          study.and.cycle <- sub(pattern = "\\.$", replacement = "", x = study.and.cycle)
        ...
        } else if(any(sapply(X = ISO, FUN = nchar) > 13 )) {
          full.inp.file.path <- file.path(inp.folder, ISO)
        ...
        study.and.cycle <- unique(x = tolower(substr(x = files, start = 7, stop = 9)))
        study.and.cycle <- sub(pattern = "\\.$", replacement = "", x = study.and.cycle)
        if(length(x = tolower(study.and.cycle)) > 1) {
          message("\nError:\nThe input folder contains data files from more than one study and/or cycle. The input folder must contain files from single study and cycle only.\n\n")
          stop()
      ...
      convert.IEA <- function(inp.file, out.file, new.file) {
        ...
        } else if(inp.file.first.char %in% c("a", "b") && study.and.cycle == "m8l") {
          study.attribute <- "TIMSS Longitudinal"
          cycle.attribute <- "2023"
