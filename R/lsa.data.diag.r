...
#' }
#'
#' @seealso \code{\link{lsa.convert.data}, \link{lsa.vars.dict}}
#' @export
...
lsa.data.diag <- function(data.file, data.object, split.vars, variables, weight.var, cont.freq = FALSE, include.missing = FALSE, output.file, open.output = TRUE, ...) {
  ...
  message("\nAlmanacs for a total of ", length(variables), " variables will be produced.")
  ...
  if(!missing(split.vars) && length(split.vars) > 1 && class(data.object) == "lsa.data" && "no.CNT.split" %in% names(further.args) && further.args[["no.CNT.split"]] != TRUE) {
    message("The almanacs for each variable will be split by country ID and ", length(split.vars) - 1, " other variables.\n")
    ...
    message("The almanacs for each variable will be split by country ID.\n")
    ...
    message("Almanacs will not be split by any variables.\n")
  } else if(!missing(split.vars) && class(data.object) == "lsa.data" && "no.CNT.split" %in% names(further.args) && further.args[["no.CNT.split"]] == TRUE) {
    message("The almanacs for each variable will be split by ", length(split.vars), " variables.\n")
